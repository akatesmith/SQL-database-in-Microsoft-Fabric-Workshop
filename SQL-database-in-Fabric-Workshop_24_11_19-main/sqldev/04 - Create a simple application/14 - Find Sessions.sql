create or alter procedure [find_sessions]
@text nvarchar(max),
@top int = 10,
...
exec @retval = get_embedding @text, @qv output;
...
(
    ...
    from 
        sessions_speakers ss 
    inner join 
        speakers sp on sp.id = ss.speaker_id 
    group by 
        session_id
)
select top(@top)
...
from 
    cteSimilar2 r
inner join 
    sessions a on r.session_id = a.id
where   
